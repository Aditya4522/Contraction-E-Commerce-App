...
export default function ProductCard({
  ...
  return (
      ...
      <div className="relative boder w-fit overflow-clip grid z-1 hover:shadow-md  ">
        <img
          src={image.url}
          alt={name}
          className=" rounded-md hover:opacity-75 cursor-pointer transition-all ease-out  h-80 w-96 hover:shadow-md  object-cover "
          h-80
        />
        <div className=" rounded-md px-3 grid gap-1 py-2 absolute bg-white dark:bg-zinc-900 w-full bottom-0  translate-y-[3rem] hover:translate-y-0 transform transition-all ease-in-out duration-300">
            ...
            <div className="flex">
                <RatiingGenrater rating={3} size={24}/>
            </div>
            <span className="flex"><h1 className="font-bold"> ₹ </h1> {price}</span>

        </div>
        
        <LInkBuuton to="/product/:id" text="View Product"/>
        </div>
      </div>
